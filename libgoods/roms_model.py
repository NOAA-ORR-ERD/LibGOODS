...
from libgoods import curv_model
...
class roms(curv_model.curv):
    ...
    def write_nc(self,var_map=None,ofn='test.nc',t_index=None, d_index=-1,is3d=False,grid_only=False,dl=0):       
        '''
            ...
            for var in data_vars:
                ...
                if is3d:
                    ...
                    if coords.find('lon_u')>-1:  
                        var_out = nc_out.createVariable(var, var_in.dtype, dims)
                        # Copy data:
                        var_out[:] = var_in[t_index[0]:t_index[1]:t_index[2],d_index,self.y[0]:self.y[1],self.x[0]:self.x[1]-1]
                    elif coords.find('lon_v')>-1:          
                        var_out = nc_out.createVariable(var, var_in.dtype, dims)
                        # Copy data:
                        var_out[:] = var_in[t_index[0]:t_index[1]:t_index[2],d_index,self.y[0]:self.y[1]-1,self.x[0]:self.x[1]]
                    else:
                        print('Variable dimensions could not be determined - skipping',var)
                        continue
