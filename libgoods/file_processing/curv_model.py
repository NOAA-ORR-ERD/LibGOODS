...
class curv(base.nc):
    ...
    def subset(self, bbox, stride=1, dl=False):
        """

        new_bbox = [wlon, slat, elon, nlat]
        ...
        if (abs(np.nanmax(glat) - nl) < 1e-3) and (
            abs(np.nanmin(glon) - wl) < 1e-3
        ):  # original values
            ...
            if not dl:
                ...
                [yvec, xvec] = np.where(
                    np.logical_and(
                        np.logical_and(glat >= sl, glat <= nl),
                        np.logical_and(glon >= wl, glon <= el),
                    )
                )
            else:
                [yvec, xvec] = np.where(
        ...
        for var in grid_vars:
            if var in var_map:
                ...
                if var in [
                    "lon",
                    "lonc",
                ]:  # Need to only do this when not crossing -180
                    ...
                    if len(var_in.shape) == 4:
                        if flipud:
                            for i, tt in enumerate(
                                range(t_index[0], t_index[1], t_index[2])
                            ):
                                var_out[i, :] = np.flipud(
                                    var_in[
                                        tt,
                        ...
                        if flipud:
                            for i, tt in enumerate(
                                ...
                                var_out[i, :] = np.flipud(
                                    var_in[
                                        tt, self.y[0] : self.y[1], self.x[0] : self.x[1]
                                    ]
                                )
                        else:
                            var_out[:] = var_in[
