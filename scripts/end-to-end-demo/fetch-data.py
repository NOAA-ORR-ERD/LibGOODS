...
def fetch_data(config: ModelConfig, output_dir: Path) -> Path:
    ...
    ds_small = ds_model.em.filter(config.standard_names).em.sub_grid(bbox=bbox)
    fname = (
        f"{config.model_name}_{config.timing}_"
...
def run_fetch_data(start_date: pd.Timestamp, end_date: pd.Timestamp, output_dir: Path):
